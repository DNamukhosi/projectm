    ...
    $rho = "0";
     $date =date('Y-m-d');
      ...
      $write1 =$conn->query("update `beds` set occupied='$rho' where b_id='$wr[bed]'")  or die(mysqli_error());
       ...
       //bed
        $wr1 =$conn->query("update `bed_trans` set discharged='$rhos' where bed='$wr[bed]' and  p_id='$id'")  or die(mysqli_error());
       //wards
        $write3 =$conn->query("update `words` set available=available+'$rhos' where w_id='$wr[ward]'")  or die(mysqli_error());
         $write4 =$conn->query("update `words` set available=available-'$rhos' where w_id='$fn'")  or die(mysqli_error());

 $wr =$conn->query("insert into `bed_trans`(p_id,name,date,ward,bed,admitted,discharged) values ('$id','$name','$date','$fn','$ln','$rhos','$rho')") or die(mysqli_error());
